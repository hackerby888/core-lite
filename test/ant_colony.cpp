...
#include "../src/extensions/ant_colony_rollback.h"
...
// ---------------------------------------------------------------------------
// Tick rollback (src/extensions/ant_colony_rollback.h)
//
// The property under test: undoing a tick's commits must leave the colony byte-identical to what it
// was before them, so a re-run reaches the same verdict from the same state. Anything the undo misses
// shows up here rather than as a digest split against quorum.

#if LITE_ANT_TICK_ROLLBACK

// The node's flag bitmap lives in qubic.cpp; restore() takes it as a parameter so a test can supply
// its own.
static constexpr unsigned int TEST_ANT_FLAG_WORDS = 64;
static unsigned long long gTestAntFlags[TEST_ANT_FLAG_WORDS];

static void setTestFlagBit(unsigned int index)
{
    gTestAntFlags[index >> 6] |= 1ULL << (index & 63);
}

static bool testFlagBit(unsigned int index)
{
    return (gTestAntFlags[index >> 6] & (1ULL << (index & 63))) != 0;
}

static const unsigned int TEST_ROLLBACK_TICK = TEST_PUBLISH_TICK + 1;

TEST(TestAntColonyRollback, RestoresTheTreeExactly)
{
    AntColonyBpp9000T* colony = freshColony();
    ASSERT_NE(colony, nullptr) << "colony init failed; needs ~6.2 GB";

    const m256i me = makeKey(1);
    const long long keep = commitRootChild(colony, me, 3800, 0, 500);
    ASSERT_NE(keep, ANT_INVALID_INDEX);

    const unsigned int countBefore = colony->solutionCount();
    const AntSolutionRecord keptBefore = *colony->recordAt(keep);
    const unsigned long long acceptedBefore = colony->stats().acceptedSolutions;

    gAntTickRollback.capture(*colony, TEST_ROLLBACK_TICK);

    ASSERT_NE(commitRootChild(colony, me, 3700, 0, 600, TEST_ROLLBACK_TICK), ANT_INVALID_INDEX);
    ASSERT_NE(commitRootChild(colony, me, 3600, 1, 601, TEST_ROLLBACK_TICK), ANT_INVALID_INDEX);
    ASSERT_GT(colony->solutionCount(), countBefore);

    gAntTickRollback.restore(*colony, gTestAntFlags);

    EXPECT_EQ(colony->solutionCount(), countBefore);
    ASSERT_NE(colony->recordAt(keep), nullptr);
    EXPECT_EQ(memcmp(colony->recordAt(keep), &keptBefore, sizeof(keptBefore)), 0)
        << "a surviving record was disturbed";
    EXPECT_EQ(colony->stats().acceptedSolutions, acceptedBefore) << "diagnostics kept the discarded pass";
    EXPECT_EQ(colony->findIndexBySolutionRef(SolutionRef{ TEST_ROLLBACK_TICK, 0 }), ANT_INVALID_INDEX)
        << "a rolled-back record still resolves";
}

// The replay key has to be released, or the re-run rejects the very solution it is replaying.
TEST(TestAntColonyRollback, ReleasesDedupOfRolledBackSolutions)
{
    AntColonyBpp9000T* colony = freshColony();
    ASSERT_NE(colony, nullptr) << "colony init failed; needs ~6.2 GB";
    ...
    gAntTickRollback.capture(*colony, TEST_ROLLBACK_TICK);
    ASSERT_NE(commitRootChild(colony, me, 3750, 0, 701, TEST_ROLLBACK_TICK), ANT_INVALID_INDEX);
    gAntTickRollback.restore(*colony, gTestAntFlags);

    // Same triple as the rolled-back commit: it must be admissible again.
    EXPECT_NE(commitRootChild(colony, me, 3750, 0, 701, TEST_ROLLBACK_TICK), ANT_INVALID_INDEX)
        << "the rolled-back solution is still deduped";

    // A key from before the capture point must stay deduped.
    EXPECT_EQ(commitRootChild(colony, me, 3800, 1, 700), ANT_INVALID_INDEX)
        << "the undo released a key it never added";
}

// commit() head-inserts, so the undo has to walk newest-first to put the chain back.
TEST(TestAntColonyRollback, RestoresTheSiblingChain)
{
    AntColonyBpp9000T* colony = freshColony();
    ASSERT_NE(colony, nullptr) << "colony init failed; needs ~6.2 GB";

    const m256i me = makeKey(3);
    const long long a = commitRootChild(colony, me, 3800, 0, 800);
    const long long b = commitRootChild(colony, me, 3810, 1, 801);
    const long long c = commitRootChild(colony, me, 3820, 2, 802);
    ASSERT_NE(a, ANT_INVALID_INDEX);
    ASSERT_NE(b, ANT_INVALID_INDEX);
    ASSERT_NE(c, ANT_INVALID_INDEX);

    gAntTickRollback.capture(*colony, TEST_ROLLBACK_TICK);
    ASSERT_NE(commitRootChild(colony, me, 3825, 0, 803, TEST_ROLLBACK_TICK), ANT_INVALID_INDEX);
    ASSERT_NE(commitRootChild(colony, me, 3830, 1, 804, TEST_ROLLBACK_TICK), ANT_INVALID_INDEX);
    gAntTickRollback.restore(*colony, gTestAntFlags);
    ...
    // The head is what commit() reads as the new node's nextSiblingIdx, so a fresh child reports it.
    const long long probe = commitRootChild(colony, me, 3790, 3, 890, TEST_ROLLBACK_TICK);
    ASSERT_NE(probe, ANT_INVALID_INDEX);
    EXPECT_EQ(colony->recordAt(probe)->nextSiblingIdx, (unsigned int)c) << "chain head was not restored";
}

// A head map entry that did not exist before the tick has to be removed, not left pointing at a
// truncated index - that would silently disable the identity's child cap.
TEST(TestAntColonyRollback, RemovesAHeadItCreated)
{
    AntColonyBpp9000T* colony = freshColony();
    ASSERT_NE(colony, nullptr) << "colony init failed; needs ~6.2 GB";

    const m256i newcomer = makeKey(4);

    gAntTickRollback.capture(*colony, TEST_ROLLBACK_TICK);
    ASSERT_NE(commitRootChild(colony, newcomer, 3800, 0, 900, TEST_ROLLBACK_TICK), ANT_INVALID_INDEX);

    gAntTickRollback.restore(*colony, gTestAntFlags);

    // With the head removed, the newcomer's next child must start a chain rather than link to the
    // truncated index the discarded pass left.
    const long long probe = commitRootChild(colony, newcomer, 3800, 0, 901, TEST_ROLLBACK_TICK);
    ASSERT_NE(probe, ANT_INVALID_INDEX);
    EXPECT_EQ(colony->recordAt(probe)->nextSiblingIdx, NO_SIBLING) << "dangling chain head left behind";
}

// Both bits of a solution are set unconditionally, and either may already belong to a different
// solution that collided on it. Clearing blind would un-see that one.
TEST(TestAntColonyRollback, ClearsOnlyTheBitsItSet)
{
    AntColonyBpp9000T* colony = freshColony();
    ASSERT_NE(colony, nullptr) << "colony init failed; needs ~6.2 GB";

    setMem(gTestAntFlags, sizeof(gTestAntFlags), 0);
    setTestFlagBit(10);   // an earlier solution owns this bit

    gAntTickRollback.capture(*colony, TEST_ROLLBACK_TICK);
    gAntTickRollback.noteFlagSet(10, true);    // collided with the earlier one
    gAntTickRollback.noteFlagSet(11, false);   // genuinely ours
    setTestFlagBit(11);

    gAntTickRollback.restore(*colony, gTestAntFlags);

    EXPECT_TRUE(testFlagBit(10)) << "un-saw a solution the tick did not introduce";
...
TEST(TestAntColonyRollback, DisarmedRestoreIsANoOp)
{
    AntColonyBpp9000T* colony = freshColony();
    ASSERT_NE(colony, nullptr) << "colony init failed; needs ~6.2 GB";

    const m256i me = makeKey(5);
    ASSERT_NE(commitRootChild(colony, me, 3800, 0, 1000), ANT_INVALID_INDEX);
    const unsigned int countBefore = colony->solutionCount();
