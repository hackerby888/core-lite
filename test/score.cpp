...
// Worker threads for the parallel path; 0 uses hardware_concurrency.
...
static unsigned int workerThreadCount(unsigned long long numSamples)
{
    ...
    const unsigned int chosen = TEST_NUMBER_OF_THREADS == 0 ? hw : std::min(hw, TEST_NUMBER_OF_THREADS);
    return std::max(1u, (unsigned int)std::min(numSamples, (unsigned long long)chosen));
}
