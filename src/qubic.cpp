...
static void processTick(unsigned long long processorNumber)
{
    ...
    if (nextTickData.epoch == system.epoch)
    {
        ...
        bool isThisTickHasAntSolution = false;
        ...
        for (unsigned int transactionIndex = 0; transactionIndex < NUMBER_OF_TRANSACTIONS_PER_TICK; transactionIndex++)
        {
            if (!isZero(nextTickData.transactionDigests[transactionIndex]))
            {
                if (tsCurrentTickTransactionOffsets[transactionIndex])
                {
                    ...
                    const bool isLegacySolutionTx = MiningSolutionTransaction::isSolutionTransaction(transaction);
                    const bool isAntSolutionTx = AntColonyMiningSolutionTransaction::isSolutionTransaction(transaction);
                    if (isLegacySolutionTx || isAntSolutionTx)
                    {
                        isThisTickHasSolution |= isLegacySolutionTx;
                        isThisTickHasAntSolution |= isAntSolutionTx;
                ...
                if (curTimeTick - loggingTick >= frequency)
                {
                    ...
                    if (gTickTotalNumberOfComputors - gTickNumberOfComputors >= QUORUM && numberOfKnownNextTickTransactions == numberOfNextTickTransactions)
                    {
                        ...
                        } else
                        {
                            ...
                            appendNumber(message, score->pendingTaskCount(), TRUE);
                            appendText(message, L".");
                            logToConsole(message);
                        }
