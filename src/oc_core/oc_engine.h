...
public:
    ...
    {
        return MAX_OC_INVOCATIONS_PER_EPOCH * sizeof(OcInvocationRecord)
            + OC_REQUEST_STORAGE_SIZE
            + MAX_OC_IN_FLIGHT_INVOCATIONS * sizeof(OcInFlightAuthState)
            + MAX_OC_IN_FLIGHT_INVOCATIONS * sizeof(int64_t)
            + sizeof(QPI::HashMap<int64_t, uint32_t, MAX_OC_INVOCATIONS_PER_EPOCH>);
    }
