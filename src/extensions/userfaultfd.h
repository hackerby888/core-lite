    ...
    UserFaultFD()
    {
        ...
        if (fd < 0)
        {
            throw std::runtime_error(
                "Error: userfaultfd syscall failed, make sure you have root privileges | Line: "
        ...
        {
            throw std::runtime_error(
                "Error: UFFDIO_API ioctl failed | Line: "
                + std::to_string(__LINE__));
        }
        ...
        {
            printf(
                "Your kernel doesn't support %s, which is required for "
                "write-protect page fault handling. Please upgrade to Linux "
                "kernel 6.5 or later.\n",
                featureName);
            printf(
                "Run: sudo apt update && sudo apt install "
                "linux-generic-hwe-22.04 -y to get a newer kernel on Ubuntu\n");
            throw std::runtime_error(
                std::string("Error: ") + featureName
                + " not supported | Line: " + std::to_string(__LINE__));
        };
